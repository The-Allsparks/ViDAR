...
/**
 * Latest-frame mailbox: portal thread writes {@code pending}; worker reads {@code processing}
 * via buffer swap (one SDK copy on publish, no copy on take).
 */
final class VidarFrameMailbox {
    ...
    Snapshot tryTake(int lastProcessedGeneration) {
        synchronized (lock) {
            ...
            return new Snapshot(processing, captureTimeNanos, generation);
        }
    }
